...
(function ($) {
    ...
    $.fn.dpsTransition = function (options) {
        var opts = $.extend({}, $.fn.dpsTransition.defaults, options);
    ...
    function Card(myCard,opts) {
        ...
        this.hScale = winH / h;
        this.wScale = winW / w;
        this.xTrans = (winW - w) / 2 - x;
        this.yTrans = (winH - h) / 2 - y + opts.header;
    ...
    Card.prototype.dpsExpand = function () {
        ...
        $('#cardWrapper').on('animationend webkitAnimationEnd',function(e){
            if(e.originalEvent.animationName=='canvas-expand'){
                ...
                $(cardObj.target).trigger('dps.expanded');
            };
        });
    }
    
    Card.prototype.dpsContract = function () {
        $('#cardWrapper').remove();
